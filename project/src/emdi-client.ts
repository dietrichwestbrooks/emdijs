...
import { Subject, of, Subscription, BehaviorSubject, empty, Observable } from 'rxjs';
...
export class EmdiClient {
  ...
  get error$(): Observable<EmdiError> {
  ...
  get disconnected$(): Observable<boolean> {
    return this._disconnected$.asObservable().pipe(filter(disconnected => !!disconnected), map(disconnected => !!disconnected));
  }
  ...
  connect(deviceId: number, accessToken: number): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      try {
        ...
        this.messages = <Subject<string>>this.service.connect(deviceId).pipe(
          map(message => {
